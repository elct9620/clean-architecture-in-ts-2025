  ...
  switch (action.type) {
    ...
    case 'UPDATE_LAST_MESSAGE':
      return state.map((msg, index) => 
        index === state.length - 1 ? { ...msg, content: msg.content + action.payload } : msg
      );
    default:
      return state;
  }
...
async function doChat(message: string, dispatcher: (type: string, payload: any) => void) {
...
export const Chat: FC = () => {
  ...
  const handleMessageDispatch = useCallback((type: string, payload: any) => {
